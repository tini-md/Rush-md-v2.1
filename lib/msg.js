const { proto, downloadContentFromMessage, getContentType } = require('@whiskeysockets/bot')
const fs = require('fs')
...
const sms = (conn, m) => {
	...
	if (m.message) {
		...
		if (m.msg) {
			...
			var quotedMention = m.msg.contextmd != null ? m.msg.contextmd.participant : ''
			var tagMention = m.msg.contextmd != null ? m.msg.contextmd.mentionedJid : []
			...
			m.quoted = m.msg.contextmd != undefined ? m.msg.contextmd.quotedMessage : null
			if (m.quoted) {
				m.quoted.type = getContentType(m.quoted)
				m.quoted.id = m.msg.contextmd.stanzaId
				m.quoted.sender = m.msg.contextmd.participant
				...
				var quoted_quotedMention = m.quoted.msg.contextmd != null ? m.quoted.msg.contextmd.participant : ''
				var quoted_tagMention = m.quoted.msg.contextmd != null ? m.quoted.msg.contextmd.mentionedJid : []
				...
				m.quoted.fakeObj = proto.WebMessagemd.fromObject({
					key: {
						remoteJid: m.chat,
	...
	m.reply = (teks, id = m.chat, option = { mentions: [m.sender] }) => conn.sendMessage(id, { text: teks, contextmd: { mentionedJid: option.mentions } }, { quoted: m })
	m.replyS = (stik, id = m.chat, option = { mentions: [m.sender] }) => conn.sendMessage(id, { sticker: stik, contextmd: { mentionedJid: option.mentions } }, { quoted: m })
	m.replyImg = (img, teks, id = m.chat, option = { mentions: [m.sender] }) => conn.sendMessage(id, { image: img, caption: teks, contextmd: { mentionedJid: option.mentions } }, { quoted: m })
	m.replyVid = (vid, teks, id = m.chat, option = { mentions: [m.sender], gif: false }) => conn.sendMessage(id, { video: vid, caption: teks, gifPlayback: option.gif, contextmd: { mentionedJid: option.mentions } }, { quoted: m })
	m.replyAud = (aud, id = m.chat, option = { mentions: [m.sender], ptt: false }) => conn.sendMessage(id, { audio: aud, ptt: option.ptt, mimetype: 'audio/mpeg', contextmd: { mentionedJid: option.mentions } }, { quoted: m })
	m.replyDoc = (doc, id = m.chat, option = { mentions: [m.sender], filename: 'undefined.pdf', mimetype: 'application/pdf' }) => conn.sendMessage(id, { document: doc, mimetype: option.mimetype, fileName: option.filename, contextmd: { mentionedJid: option.mentions } }, { quoted: m })
	m.replyContact = (name, md, number) => {
